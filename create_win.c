...
/*   Updated: 2022/02/24 19:42:39 by teambersaw       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_image	ft_init_image(void *mlx, int var, char **map, void *mlx_win)
{
	...
	image.perso = NULL;
	image.objet = NULL;
	image.floor = mlx_xpm_file_to_image(mlx, name[0], &var, &var);
	image.wall = mlx_xpm_file_to_image(mlx, name[1], &var, &var);
	image.objet = mlx_xpm_file_to_image(mlx, name[2], &var, &var);
	image.exit = mlx_xpm_file_to_image(mlx, name[3], &var, &var);
	image.perso = mlx_xpm_file_to_image(mlx, name[4], &var, &var);
	image.exiton = mlx_xpm_file_to_image(mlx, name[5], &var, &var);
	ft_free(name, NULL);
